...
import {customtest} from "../utils/datafixture"


customtest(` End to end order with test data fixture`, async ({ page,testDataForOrder }) => {
    ...
    await db.addProductToCart(testDataForOrder.productName)
    // go to cart page
    await db.goToCartPage()
    
    //in cartpage
    await cp.verifyCartItem(testDataForOrder.productName)
    await cp.checkout()
